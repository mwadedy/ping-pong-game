...
import turtle
wind = turtle.Screen()  # intialize sceern
wind.title("Ping Pong By WADEDY")  # set the title of the window
...
wind.tracer(0)  # stops the window form updating automatically
...
madrab1.color("green")  # set the color of the shape
# stretch the shapte to meet the size
...
ball.penup()
ball.goto(0, 0)
ball.dx = 0.5  # set the ball speed
ball.dy = 0.5
...
score.hideturtle()
score.goto(0, 260)
score.write("Player I: 0 Player II: 0", align="center",
...
# keyboard bindings
wind.listen()  # tell the window to expect keyborad input
# when pressing w the function madrab1_up is invoked
...
while True:
    ...
    # move the ball
    # ball starts at 0 and everytime loops run--->+0.5 xaxis
    ball.setx(ball.xcor() + ball.dx)
    # ball starts at 0 and everytime loops run--->+0.5 yaxis
    ...
    if ball.ycor() > 290:  # if ball is at top border
        ...
        ball.dy *= -1  # revese direction, making +0.5--->-0.5
    ...
    if ball.xcor() > 390:  # if ball is at right border
        ...
        score1 += 1
        score.clear()
        score.write("Player I: {} Player II: {}".format(score1, score2), align="center",
                    font=("Courier", 24, "normal"))
